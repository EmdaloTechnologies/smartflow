...
typedef struct _PinDescription
{
	...
	AnalogChannel  adcChannelNumber; /* ADC Channel number in the SAM device */
	PWMChannel     pwmChannel;
	TCChannel      tcchannel;
	Ext_Interrupts extInt;
} PinDescription;
...
//#define PIN_SERIAL0_TX        (55ul)
//#define PIN_SERIAL0_RX        (56ul)
//#define PAD_SERIAL0_TX        (UART_TX_PAD_0) // (UART_TX_RTS_CTS_PAD_0_2_3)
...
int pinPeripheral(uint32_t pin, PioType peripheral);

void digitalWrite(uint32_t pin, uint32_t val);
int digitalRead(uint32_t pin);

//void Uart_begin(unsigned long baudrate, uint16_t config,int uc_pinRX,int uc_pinTX,int uc_padRX,int uc_padTX);
